...
class TestProfileEditModal(IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.profile_edit_modal = ProfileEditModal(player_mock, player_repo_mock)

        self.response_mock = AsyncMock(InteractionResponse, **{"is_done.return_value": False})
        self.interaction_mock = Mock(Interaction, **{"response": self.response_mock})


    def test_is_initialized_properly(self):
        self.assertHasAttr(self.profile_edit_modal, "title")
        self.assertEqual(self.profile_edit_modal.title,
                         f" Edit {self.player_name} Profile Details")

        self.assertHasAttr(self.profile_edit_modal, "timeout")
        self.assertIsNone(self.profile_edit_modal.timeout)


    async def test_can_process_submit_callback_with_no_changes(self):
        await self.profile_edit_modal.on_submit(self.interaction_mock)
